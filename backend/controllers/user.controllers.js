...
if(isFollowing){
  ...
  await currentUser.save()
   ...
   // live-update: tell both users' open tabs/sessions the follow state changed
   const unfollowPayload={followerId:currentUser._id,followingId:targetUser._id,isFollowing:false}
   const targetSocketId=getSocketId(targetUser._id)
   if(targetSocketId){ io.to(targetSocketId).emit("followUpdate",unfollowPayload) }
   const currentSocketId=getSocketId(currentUser._id)
   if(currentSocketId){ io.to(currentSocketId).emit("followUpdate",unfollowPayload) }
 ...
 targetUser.followers.push(currentUserId)
  ...
  await currentUser.save()
   ...
   // live-update: tell both users' open tabs/sessions the follow state changed
   const followPayload={followerId:currentUser._id,followingId:targetUser._id,isFollowing:true}
   const targetSocketId2=getSocketId(targetUser._id)
   if(targetSocketId2){ io.to(targetSocketId2).emit("followUpdate",followPayload) }
   const currentSocketId2=getSocketId(currentUser._id)
   if(currentSocketId2){ io.to(currentSocketId2).emit("followUpdate",followPayload) }
